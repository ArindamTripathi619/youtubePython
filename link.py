...
def download_playlist(link):
    try:
        ydl_opts = {'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/mp4', 'outtmpl': '%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s'}
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ydl.download([link])
    except Exception as e:
...
def get_user_input():
    choice = input("What do you want to download? Enter 1 for single video, 2 for multiple videos, or 3 for a playlist: ")
    if choice == '1':
